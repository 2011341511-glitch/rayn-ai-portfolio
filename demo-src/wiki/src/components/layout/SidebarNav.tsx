...
import { BarChart3, Home, LogOut, MessageSquareQuote, BookOpen } from 'lucide-react';
...
const NAV_ITEMS: NavItem[] = [
  { key: 'home', label: '首页', to: '/', icon: Home, exact: true, disabled: true },
  { key: 'chat', label: '问股', to: '/chat', icon: MessageSquareQuote, disabled: true },
  { key: 'wiki', label: 'LLM Wiki', to: '/wiki', icon: BookOpen },
]
  ...
  return (
    <div className="flex h-full flex-col">
      <div className={cn('mb-4 flex items-center gap-2 px-1', collapsed ? 'justify-center' : '')}>
        ...
        {!collapsed ? (
          <p className="min-w-0 truncate text-sm font-semibold text-foreground">Stock Analysis</p>
        ) : null}
      </div>

      <nav className="flex flex-1 flex-col gap-1.5" aria-label="主导航">
        {NAV_ITEMS.map(({ key, label, to, icon: Icon, exact, disabled }) => {
          const itemClass = cn(
          ...
          if (disabled) {
            return (
              <button
                ...
                type="button"
                disabled
                aria-label={`${label}（公开版已屏蔽）`}
                title="公开版仅展示 LLM Wiki"
                className={cn(itemClass, 'cursor-not-allowed border-transparent text-secondary-text/45')}
              >
                <Icon className="ml-1 h-5 w-5 shrink-0 text-current" />

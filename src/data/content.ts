...
export const projects: ProjectCase[] = [
  ...
  },
  {
    ...
    summary: '将 FinClaw 桌面端完整复刻为可在线预览的 Web 原型，并沉淀首次建设与增量跟版两套复用流程。',
    problem: '桌面端产品在跨端评审、产品演示与快速验证中缺少可直接访问的体验载体；手工逐页复刻难以兼顾视觉交互一致性与后续版本跟进。',
    workflow: ['解析桌面端页面与交互', '以 Mock 方式适配 Skill 调用', '生成全功能网页预览', '构建验收与增量跟版'],
    outcomes: ['支持可公开功能与完整交互链路的 Web 预览，Skill 调用使用本地 Mock 数据模拟', '实现桌面端与网页端 1:1 的视觉及交互复刻，形成可用于评审、演示与验证的高保真产品原型', '沉淀首次复刻与增量跟版两套 Skill，支持离线交付与后续版本同步'],
    metrics: [
      { value: '全功能', label: '网页预览' },
      { value: 'Mock', label: 'Skill 调用' },
      { value: '1:1', label: '桌面端复刻' },
